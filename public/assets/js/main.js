...
var defaultSettings = {
    ...
    templateExtension: 'jtmp',
    partialPath: 'template/partials',
    partialExtension: 'partial',
    partials: ['header', 'footer'],
    showMsg: function (data, textStatus, jqXHR) {
        if (data.code !== 200) {
...
(function ($) {
  ...
  function registerPartial(path, name) {
    ...
    $.fn.renderAppend = function (templateName, data, callback) {
        var $this = this;
        var template = Handlebars.compile(templateName);
        console.log(template(data))
        $this.append(template(data)).promise().done(callback);
        return this;
    };
}(jQuery));
function ajaxPost(url, ajaxData, callback) {
    $.ajax({
        type: 'POST',
        url: url,
        data: ajaxData,
        ...
        error: function(xhr, status, error) {
            alertMsg(xhr.responseJSON.message, 'error');
        }
    });
}
function alertMsg(Message, Type, callback) {
    if (Type == 'error') {
        Swal.fire({
            titleText: Message,
            type: Type,
            customClass: {
                title: 'shop-title-class',
            },
        }).then(callback);
    } else {
        Swal.fire({
...
function alertExtendMsg(p, title, Type, callback) {
...
function isLogin() {
    return setting_shop.isLogin;
}
function getCartNum() {
    $.getJSON(setting_shop.cartnum, function (data) {
        if (data.code == 200) {
            $('#cart_num').html(data.data.num);
            $('#cart_num_x').html(data.data.num);
        }
    });
}
function addCart(goodsId, num, skuid, options, callback) {
    if (isLogin() == false) {
        alertMsg('请登陆或注册后再操作', 'error', function () {
		...
		console.log(options);
        ajaxPost(setting_shop.cartput, { 'num': num, 'goodsId': goodsId, 'skuId': skuid, 'options': options }, function (data) {
            if (data.code !== 200) {
                alertMsg(data.message, 'error');
            } else {
                showToast('已经加入了你的购物袋');
                getCartNum()
            }
        });
    }
}
function takeCart(goodsId, num, skuid, callback) {
    if (isLogin() == false) {
        alertMsg('请登陆或注册后再操作', 'error', function () {
            window.location.href = '/Index/Login';
        });
        ...
        ajaxPost(setting_shop.carttake, { 'num': num, 'goodsId': goodsId, 'skuId': skuid }, function (data) {
            if (data.code !== 200) {
                alertMsg(data.message, 'error');
            } else {
                showToast('减少了该款数量');
...
function deleteCart(goodsId, skuid, callback) {
    if (isLogin() == false) {
        alertMsg('请登陆或注册后再操作', 'error', function () {
            window.location.href = '/Index/Login';
        });
        ...
        ajaxPost(setting_shop.cartdelete, { 'goodsId': goodsId, 'skuId': skuid }, function (data) {
            if (data.code !== 200) {
                alertMsg(data.message, 'error');
            } else {
                showToast('移除了这款商品');
